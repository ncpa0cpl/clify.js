...
var import_compare_strings = require("../Utils/compare-strings.cjs");
var import_errors = require("../Utils/errors.cjs");
var import_argument_parser = require("./argument-parser.cjs");
var alphanumComparator = (0, import_compare_strings.compareStrings)({ alphanum: true });
...
var Argument = class _Argument {
  ...
  static getArgumentsInfo() {
    return import_argument_parser.Arguments.getRegisteredArguments().filter(
  ...
  static define(initData) {
    ...
    class Arg extends _Argument {
      constructor() {
        super();
        import_argument_parser.Arguments.registerArgument(this);
      ...
      static access() {
        if (!Arg.instanceRef) {
          ...
          `);
          throw new import_errors.ClifyError("Accessing uninitialized argument.");
        }
  ...
  validate() {
    ...
    try {
      (0, import_dilswer.assertDataType)(
        this.context.dataType ?? import_dilswer.Type.Unknown,
        this._value
      );
    } catch (err) {
      if (import_dilswer.ValidationError.isValidationError(err)) {
  ...
  getInitialContext() {
    throw new import_errors.ClifyError("Unreachable");
  }
  static access() {
    throw new import_errors.ClifyError("Unreachable");
  }
